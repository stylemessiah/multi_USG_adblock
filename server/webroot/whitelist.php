...
$contents = file($whitelist, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
...
//process form submission for adddomain (adding a domain to whitelist)
...
<section class="section-table cid-r65YSca07c" id="table1-8">
  <div class="container container-table">
      <h2 class="mbr-section-title mbr-fonts-style align-center pb-3 display-2">
		...
		<label for="usr">Add a new site to the whitelist:</label>&nbsp;&nbsp;&nbsp;
		<div class="input-group input-group-sm col-6">
		<input type="text" class="form-control" placeholder="type a domain name" name="adddomain"></input>
		<button type="submit" class="btn btn-primary p-1">Whitelist (Unblock) this site</button>
		</div>
		</form>
		<p>&nbsp;</p>
		<div class="container">
          ...
          <table class="table isSearch" id="domains" cellspacing="0">
			...
			if(is_array($contents)){
				foreach ($contents as $item) {
					echo '<tr><td class="body-item mbr-fonts-style display-11"><button type="submit" class="btn btn-primary btn-sm p-1" name="id" id="id" value="'.$item.'"/>Un-Whitelist</button>'.$item.'</td></tr>'."\n";
			}
			}
			?>
			</tbody>
